...
dgsCmdAddCommandHandler("server.console",function(cmd)
	if not isElement(serverConsole["window"]) then
		serverConsole["window"] = dgsCreateAnimationWindow(sW/2-400,sH/2-300,800,600,"Multi Theft Auto Server",false,tocolor(20,20,200,255),_,_,tocolor(80,140,200,255),_,tocolor(0,0,0,200))
		dgsWindowSetSizable(serverConsole["window"],false)
		dgsBringToFront(serverConsole["window"])
		outputCmdMessage(cmd,"Server Console: ON")
		dgsShowCursor(true,"console")
	else
		outputCmdMessage(cmd,"Server Console: OFF")
		dgsCloseWindow(serverConsole["window"])
		dgsShowCursor(false,"console")
	end
end)
...
addEventHandler("onDgsDestroy",resourceRoot,function()
	...
	elseif source == serverConsole["window"] then
		dgsShowCursor(false,"console")
	end
end)
...
serverConsole = {}
serverInfo = {}
addEvent("DGSI_ReceiveServerInfo",true)
function createServerConsole()
	if isElement(serverConsole.window) then
		
		serverConsole.cmd = dgsCreateCmd(0,20,800,600-65,false,serverConsole["window"],1,1)
		serverConsole.statusBar = dgsCreateImage(0,0,800,20,_,false,serverConsole["window"],tocolor(204,204,204,255))
		serverConsole.runningStatusLeft = dgsCreateLabel(0,0,20,20,"[",false,serverConsole.statusBar)
		serverConsole.runningStatus = dgsCreateLabel(8,0,20,20,"-",false,serverConsole.statusBar)
		dgsSetProperty(serverConsole.runningStatus,"functions",[[
			local tick = math.floor(getTickCount()%360/360*3)
			if tick == 0 then
				dgsElementData[self].text = "-"
			elseif tick == 1 then
				dgsElementData[self].text = "\\"
			elseif tick == 2 then
				dgsElementData[self].text = "/"
		...
		serverConsole.runningStatusRight = dgsCreateLabel(16,0,20,20,"]",false,serverConsole.statusBar)
		serverConsole.name = dgsCreateLabel(25,0,20,20,"MTA: San Andreas",false,serverConsole.statusBar)
		serverConsole.sep1 = dgsCreateLabel(155,0,20,20,"::",false,serverConsole.statusBar)
		serverConsole.players = dgsCreateLabel(175,0,20,20,"0/0 players",false,serverConsole.statusBar)
		serverConsole.sep2 = dgsCreateLabel(265,0,20,20,"::",false,serverConsole.statusBar)
		serverConsole.resources = dgsCreateLabel(285,0,20,20,"59 resources",false,serverConsole.statusBar)
		dgsSetProperties({
			serverConsole.runningStatusLeft,
			serverConsole.runningStatusRight,
			serverConsole.runningStatus,
			serverConsole.name,
			serverConsole.players,
			serverConsole.resources
		},{
			textColor = tocolor(12,12,12,255),
		...
		dgsSetProperties({
			...
			textColor = tocolor(197,15,31,255),
			alignment = {"left","center"},
			font = "default",
			textSize = {1.2,1.2},
		})
		local edit = dgsCmdGetEdit(serverConsole.cmd)
		dgsEditAutoCompleteAddParameterFunction("resource",[[
			...
			for resName,res in pairs(getElementData(root,"DGSI_Resources")) do
				if input:lower() == resName:sub(1,input:len()):lower() then
					matched[#matched+1] = resName
				end
			end
			table.sort(matched)
			return matched[1]
		]])
		outputCmdMessage(serverConsole.cmd,"Connecting...")
		dgsSetProperty(serverConsole.cmd,"enableCommandInfo",false)
		local requiredData = {
			connection = true,
			maxPlayers = true,
			resources = true,
		}
		triggerServerEvent("DGSI_RequestServerInfo",localPlayer,requiredData)
		---Add Commands
		dgsCmdAddCommandHandler(serverConsole.cmd,"help",function(cmd)
			outputCmdMessage(serverConsole.cmd,"start	stop	restart")
		end)
		
		dgsCmdAddCommandHandler(serverConsole.cmd,"start",function(cmd,resName)
			local requiredData = {
				start = resName,
			}
			triggerServerEvent("DGSI_RequestServerInfo",localPlayer,requiredData)
		...
		dgsCmdAddCommandHandler(serverConsole.cmd,"stop",function(cmd,resName)
			local requiredData = {
				stop = resName,
			}
			triggerServerEvent("DGSI_RequestServerInfo",localPlayer,requiredData)
		end)
		dgsEditAddAutoComplete(edit,{"stop","resource"},false,true)
		
		dgsCmdAddCommandHandler(serverConsole.cmd,"restart",function(cmd,resName)
			local requiredData = {
				restart = resName,
			}
			triggerServerEvent("DGSI_RequestServerInfo",localPlayer,requiredData)
		end)
		dgsEditAddAutoComplete(edit,{"restart","resource"},false,true)
		
		dgsCmdAddCommandHandler(serverConsole.cmd,"refresh",function(cmd,resName)
			local requiredData = {
				refresh = resName or true,
			}
			triggerServerEvent("DGSI_RequestServerInfo",localPlayer,requiredData)
		end)
		dgsEditAddAutoComplete(edit,{"refresh","resource"},false,true)
		
		dgsCmdAddCommandHandler(serverConsole.cmd,"refreshall",function(cmd,resName)
			local requiredData = {
				refreshall = true,
			}
			triggerServerEvent("DGSI_RequestServerInfo",localPlayer,requiredData)
		end)
		dgsEditAddAutoComplete(edit,{"refreshall"},false,true)
	end
end

function outputServerCMDConsole(cmd,time,messages)
	local formattedTime = string.format("[%02d:%02d:%02d] ", time.hour, time.minute, time.second)
	if type(messages) == "table" then
		for i=1,#messages do
			local msg = messages[i]
			outputCmdMessage(cmd,formattedTime..msg)
		end
	else
		outputCmdMessage(cmd,formattedTime..messages)
	end
end

addEventHandler("DGSI_ReceiveServerInfo",localPlayer,function(received)
	if not isElement(serverConsole.cmd) then return end
	if received == "disabled" then return outputCmdMessage(serverConsole.cmd,"Server console is disabled on this server") end
	if received == "denied" then return outputCmdMessage(serverConsole.cmd,"Access denied") end
	if received.connection == true then
		outputServerCMDConsole(serverConsole.cmd,received.time,"Server console connected")
	end
	if received.maxPlayers then
		dgsSetText(serverConsole.players,#getElementsByType("player").."/"..received.maxPlayers.." players")
	end
	if received.debugMessage then
		outputServerCMDConsole(serverConsole.cmd,received.time,received.debugMessage)
	end
	if received.restart then
		outputServerCMDConsole(serverConsole.cmd,received.time,received.restart)
	end
	if received.start then
		outputServerCMDConsole(serverConsole.cmd,received.time,received.start)
	end
	if received.stop then
		outputServerCMDConsole(serverConsole.cmd,received.time,received.stop)
	end
	if received.refresh then
		outputServerCMDConsole(serverConsole.cmd,received.time,received.refresh)
	end
	if received.refreshall then
		outputServerCMDConsole(serverConsole.cmd,received.time,received.refreshall)
	end
end)
